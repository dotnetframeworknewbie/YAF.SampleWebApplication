...
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
   ...
   <script type="text/javascript">
      ...
      $(function () {
         ...
         $("#divChatWindow").mouseover(function () { $("#MsgCountMain").html('0'); $("#MsgCountMain").attr("title", '0 New Messages'); });
      ...
      function registerClientMethods(chatHub) {
         ...
         chatHub.client.onConnected = function (id, userName, userId, allUsers, messages, times) {
            ...
            // Add All Users
            for (i = 0; i < allUsers.length; i++) { AddUser(chatHub, allUsers[i].ConnectionId, allUsers[i].UserName, allUsers[i].UserImage, allUsers[i].LoginTime); }

            // Add Existing Messages
            for (i = 0; i < messages.length; i++) { AddMessage(messages[i].UserName, messages[i].Message, messages[i].Time, messages[i].UserImage); }
         };

         // On New User Connected
         chatHub.client.onNewUserConnected = function (id, name, UserImage, loginDate) { AddUser(chatHub, id, name, UserImage, loginDate); };
         ...
         chatHub.client.sendPrivateMessage = function (windowId, fromUserName, message, userimg, CurrentDateTime) {
            ...
            if ($('#' + ctrId).length == 0) { OpenPrivateChatcard(chatHub, windowId, ctrId, fromUserName, userimg); }
            var CurrUser = $('#hdUserName').val();
            var Side = 'right';
            var TimeSide = 'left';
            if (CurrUser == fromUserName) { Side = 'left'; TimeSide = 'right'; }
            else {
            ...
            $('#' + ctrId).find('#divMessage').slimScroll({ height: ScrollHeight });
         };
      }
            ...
            <div class="card">
               ...
               <div class="card-body">
                  <div class="card-body" id="chat-card">
                     <div id="divChatWindow" class="direct-chat-messages" style="height: 450px;"></div>
                  </div>
               </div>
               <div class="card-footer">
                  <textarea id="txtMessage" class="form-control" ></textarea>
                  <div class="float-right"><input type="button" class="btn btn-primary" id="btnSendMsg" value="Send" /></div>
               </div>
            </div>
         </div>
         <div class="col-md-4">
            <div class="card">
               ...
               <div class="card-body" id="divusers"></div>
            </div>
                    
            <%--<div class="card mt-3">
               <ul class="contacts-list" id="ContactList"></ul>
            </div>--%>
         </div>
         <%--<div class="row">
            <div class="col-md-12">
   ...
   <style>
      .img-sm { width: 20px; }
      .direct-chat-text img { width: 20px; }
   </style>

   <script>
      $(function () { $("#txtMessage").emojioneArea(); });
   </script>
</asp:Content>